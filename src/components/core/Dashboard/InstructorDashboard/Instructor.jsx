...
export default function Instructor() {
  ...
  const [instructorData, setInstructorData] = useState([]); // Initialize as an empty array
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const courseResult = await fetchInstructorCourses(token);

        console.log('Instructor Data:', instructorApiData);
        console.log('Courses Data:', courseResult);

        // Ensure instructorApiData and its courses are valid
        if (instructorApiData && instructorApiData.courses) {
          setInstructorData(instructorApiData.courses); // Set instructor data courses
        }

        if (Array.isArray(courseResult) && courseResult.length > 0) {
          setCourses(courseResult); // Set additional courses data if available
        }
  ...
  // Ensure instructorData is an array before using reduce
